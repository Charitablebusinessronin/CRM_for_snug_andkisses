...
// Get a specific document template details
export async function GET(_request: NextRequest, { params }: { params: { templateId: string } }) {
  try {
    const templateId = params.templateId
    await logAuditEvent({ action: 'DOC_TEMPLATE_GET_REQUEST', details: { templateId }, hipaa: true })
    const result = await callCatalyst('documents_getTemplate', { templateId })
    return NextResponse.json({ success: true, data: result })
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error)
    await logAuditEvent({ action: 'DOC_TEMPLATE_GET_ERROR', details: { error: msg }, hipaa: true })
    return NextResponse.json({ success: false, error: 'Template retrieval failed', details: process.env.NODE_ENV==='development'?msg:undefined }, { status: 500 })
  }
}
