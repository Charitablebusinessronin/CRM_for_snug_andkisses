...
export async function GET(request: NextRequest, { params }: { params: { contractId: string } }) {
  try {
    ...
    await logAuditEvent({ action: 'CONTRACT_STATUS_REQUEST', details: { contractId }, hipaa: true })
    const result = await callCatalyst('contract_getStatus', { contractId })
    return NextResponse.json({ success: true, data: result })
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error)
    await logAuditEvent({ action: 'CONTRACT_STATUS_ERROR', details: { error: msg }, hipaa: true })
    return NextResponse.json({ success: false, error: 'Status retrieval failed', details: process.env.NODE_ENV==='development'?msg:undefined }, { status: 500 })
  }
}
